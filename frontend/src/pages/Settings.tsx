...
import PlatformFilterPicker from '../components/PlatformFilterPicker'
...
export default function Settings() {
  ...
  return (
    <div className="px-4 py-5 space-y-8">
      ...
      <section>
        <h2 className="font-display text-xl mb-1">IGDB Platform Filter</h2>
        <p className="text-xs text-text-muted mb-5">
          Limit IGDB search results to games available on your selected systems.
          Leave all unchecked (or select all) to show results for every platform.
        </p>
        <PlatformFilterPicker
          selected={settings.igdbPlatforms}
          onChange={igdbPlatforms => update({ igdbPlatforms } as Partial<AppSettings>)}
        />
      </section>
    </div>
  )
