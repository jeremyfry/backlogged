...
import { ChevronDown, Clock, RefreshCw, Upload } from 'lucide-react'
...
interface Props {
  ...
  onSubmit: (data: CreateGameInput, addAnother: boolean) => Promise<void>
  onCancel: () => void
  submitLabel?: string
  /** Show a "Save & Add Another" button that keeps the form open for the next entry. */
  allowAddAnother?: boolean
  /** Start with the "More Details" section expanded — used when editing a game that may already have those fields filled in. */
...
export default function GameForm({
  initial, igdbData, onSubmit, onCancel, submitLabel = 'Save', allowAddAnother = false, defaultExpanded = false,
}: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button
        ...
        className="flex items-center justify-between w-full text-xs text-text-muted tracking-wide uppercase py-1"
      >
      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" className="btn btn-primary flex-1" disabled={saving} onClick={() => setAddAnother(false)}>
            {saving && !addAnother ? 'Saving…' : submitLabel}
          </button>
        </div>
      </div>
    </form>
